# Schritt 7: Grafiken fuer den Bericht erzeugen
# Erstellt die ggplot2-Grafiken (Interpretationstexte in docs/03_visualisierungen.md).
# Die Windkraftdichte ist stark rechtsschief, darum bekommen die Plots 1-4 eine
# pseudo-logarithmische y-Achse (das Modell selbst bleibt unveraendert).
# Ergebnis: plots/01-08 (.png)

library(sf)
library(tidyverse)
library(scales)

# Finalen Datensatz und die gespeicherten Modellergebnisse laden
...
# Ein gemeinsames helles Layout fuer alle Plots
...
# Pseudo-log-Skala fuer die Windkraftdichte: verhaelt sich wie log10, kommt aber
# mit exakten Nullen (Kreise ganz ohne Windkraft) zurecht.
...
# --- Plot 1: Steuerkraft vs. Windkraftdichte (Scatterplot) -------------------
...
# --- Plot 2: Windkraftdichte nach Steuerkraft-Quartilen ----------------------
# Kreise in vier gleich grosse Finanzstaerke-Klassen einteilen
df_quartile <- df %>%
  mutate(
...
# --- Plot 3: Windkraftdichte nach Wirtschaftsstruktur ------------------------
...
# --- Plot 4: Kontrollvariablen (facettierte Scatterplots) --------------------
# Die vier Kontrollvariablen ins Langformat bringen, damit facet_wrap sie
# nebeneinander zeigen kann.
df_lang <- df %>%
...
# --- Plot 5: Groesste Abweichungen vom Modell (Residuen) ---------------------
# Residuen streuen um 0, darum hier keine log-Skala. Die 10 groessten positiven
# und die 10 groessten negativen Abweichungen zeigen.
...
# --- Plot 6: Modelldiagnose (Residuen vs. Fitted & Q-Q-Plot) -----------------
diag_df <- data.frame(
...
# --- Plot 7: Koeffizientenplot (Kernergebnis des Modells) --------------------
# Alle sieben Einflussfaktoren mit standardisierten Effektstaerken und
# 95%-Konfidenzintervallen. Kreuzt ein Balken die 0, ist der Effekt nicht belastbar.
...
p7 <- ggplot(koeff, aes(x = reorder(Klarname, Estimate), y = Estimate)) +
  ...
  coord_flip() +
  labs(
    # Zeilenumbruch mit \n von Hand, damit der Titel nicht aus dem Bild laeuft
    title = "Standardisierte Effektstärken\nim multivariaten Modell",
    subtitle = "Punkt = Koeffizient, Balken = 95%-Konfidenzintervall.\nKreuzt der Balken die 0, ist der Effekt nicht belastbar.",
    x = NULL, y = "Effektstärke (in Standardabweichungen)",
    caption = "Windgeschwindigkeit dominiert den Ausbau; Steuerkraft ist nicht signifikant (kreuzt die Null-Linie)."
  ) +
  theme_report()
...
# --- Plot 8: Robustheitscheck (Steuerkraft ueber drei Stichproben) -----------
# Derselbe Steuerkraft-Koeffizient, auf allen Kreisen und auf zwei
# eingeschraenkten Teilstichproben. Vergleichbar, weil ueberall dieselbe
# Variable in denselben Einheiten (kW/km² je €/Einwohner).
...
p8 <- ggplot(robustheit_plot_df, aes(x = Stichprobe_Label, y = Steuerkraft_Koeffizient)) +
  ...
  coord_flip() +
  labs(
    title = "Robustheitscheck: Steuerkraft-Effekt über drei\nStichproben",
    subtitle = "Derselbe Koeffizient (kW/km² je €/Einwohner), einmal auf allen\nKreisen und auf zwei Teilstichproben.",
    x = NULL, y = "Steuerkraft-Koeffizient (kW/km² je €/Einwohner)",
    caption = "Der Balken kreuzt in allen drei Fällen die Null-Linie: der fehlende Steuerkraft-Effekt ist robust."
  ) +
  theme_report()
